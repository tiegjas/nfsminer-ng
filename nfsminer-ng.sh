...
case $key in
	...
	echo "-fd  --  Depth of folder recursions.  Default is 3."
...
targetscan () {
	...
	then
		...
		do
			...
			if [ $fd -eq 0 ]
			then
			#Default folder depth is 3. If $fd is set by command line then the value presented by the user is accepted.
			fdepth=3
			fi
			...
			echo "--------------" >> $hostscanlog
			echo "Capturing $fdepth layers of $scantarget:$exporttarget to $hostscanlog.verbose"
			...
			echo "$fdepth layers of $scantarget:$exporttarget" >> $hostscanlog.$exportshort.verbose
			find $scandir -maxdepth $fdepth -printf "depth="%d/"sym perm="%M/"perm="%m/"size="%s/"user="%u/"group="%g/"name="%p/"type="%Y\\n 2>/dev/null >> $hostscanlog.$exportshort.verbose
